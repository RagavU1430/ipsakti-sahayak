...
def main() -> int:
    ...
    for row in rows:
        ...
        for n, item in enumerate(chunks(structural_units(result["pages"])),1):
            ...
            chunk_rows.append({"chunk_id":f"{row['source_id']}-{kind[:3]}-{ident}-{n:03d}","document_id":row["source_id"],"document_version_id":version_id,"title":row["title"],"authority":row["authority"],"jurisdiction":row["jurisdiction"],"domain":row["domain"],"subdomain":row["subdomain"],"document_type":row["document_type"],"chapter":label if kind=="CHAPTER" else None,"section":label if kind=="SECTION" else None,"subsection":None,"rule":label if kind=="RULE" else None,"article":label if kind=="ARTICLE" else None,"rule_number":number if kind=="RULE" else None,"article_number":number if kind=="ARTICLE" else None,"structure_type":kind,"structure_number":number or None,"page_start":item["page_start"],"page_end":item["page_end"],"text":item["text"],"source_url":row["source_url"],"citation_label":f"{row['short_title']} — {label}" if label else row["short_title"],"language":row["language"]})
