...
def main() -> int:
    ...
    for row in rows:
        ...
        try:
            ...
            row["notes"] = row["notes"].split(" | Download failed:", 1)[0]
            ...
            base_note = row["notes"].split(" | Download failed:", 1)[0]
            row["status"] = "FAILED"; row["notes"] = f"{base_note} | Download failed: {exc}".strip(" |"); print(f"FAILED {row['source_id']}: {exc}")
