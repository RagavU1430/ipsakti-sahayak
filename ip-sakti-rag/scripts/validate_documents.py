...
def main() -> int:
    ...
    for row in rows:
        if row["status"] not in {"DOWNLOADED","VALIDATED","EXTRACTED","PROCESSED"}: continue
        ...
        if not path.exists() or path.stat().st_size == 0: errors.append("missing or empty file")
            ...
            if not looks_like_pdf(path.read_bytes()[:8]): errors.append("invalid PDF header")
                ...
                try:
                    with fitz.open(path) as doc: page_count=len(doc); first=doc[0].get_text() if page_count else ""
                    if not first.strip(): errors.append("first page has no extractable text")
